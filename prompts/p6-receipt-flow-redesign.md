# Receipt Scan — People Picker + Items Screen Redesign

Load `graphify-out/GRAPH_REPORT.md`. Relevant communities: 25 (Mobile Navigation & Notifications), 16 (UI Primitive Components), 37 (Mobile Theme System), 59 (Mobile Pro Hub).

Three goals in one file:
1. **Fix navigation error**: `navigation.navigate('Groups')` fails from a modal screen because it can't see the tab navigator. Fix to use the root navigator correctly.
2. **Add "people" phase**: After parsing, show a "Who's splitting this?" screen before the items list.
3. **Redesign items phase**: Match the reference screenshot aesthetic — left circular checkbox, item name, price, collapsible tip row, big pay CTA.

---

## File to rewrite

`mobile/src/screens/ReceiptScanScreen.tsx` — full replacement.
...
```
idle → [camera] → parsing → people → items
```

- **idle**: Camera launch screen
- **parsing**: 2.5s mock OCR with scan animation
- **people**: "Who's splitting this?" — avatar chips, tap to include/exclude
- **items**: "Choose Your Items" — left checkboxes, per-item prices, tip row, CTA
...
import { Camera, ArrowLeft, Check, ReceiptText, Plus, ChevronRight, UserPlus } from 'lucide-react-native';
...
export default function ReceiptScanScreen({ navigation }: any) {
  ...
  const myItems    = MOCK_ITEMS.filter(i => claimed.has(i.id));
  const myFood     = myItems.reduce((s, i) => s + i.price, 0);
  ...
  // ── Navigation fix: from a modal stack screen, reach the tab navigator ────
    ...
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.idleBody}>
          ...
          <TouchableOpacity style={[styles.cameraBtn, { backgroundColor: '#10B981' }]} activeOpacity={0.82} onPress={handleOpenCamera}>
    ...
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <Animated.View style={[
          ...
          <Text style={[styles.peopleTitle, T.extrabold, { color: colors.text }]}>
              ...
              return (
                ...
                >
                  ...
                  <View style={[
                    ...
                    <Text style={[styles.memberInitial, { color: isIn ? '#fff' : m.color + 'AA' }]}>
                      {m.initial}
                    </Text>
                    {isIn && (
                  ...
                  <Text
                    style={[styles.memberName, T.semibold, { color: isIn ? colors.text : colors.tertiaryText }]}
                    numberOfLines={1}
                  >
                    {m.id === 'me' ? 'You' : m.name}
                  </Text>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ITEMS — "Choose Your Items" (screenshot-style)
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
      {/* Header */}
            ...
            <Animated.View key={item.id} style={{
              ...
              >
                ...
                <Text style={[
                  styles.itemName,
                  T.semibold,
                  { color: isClaimed ? colors.accent : colors.text, flex: 1, marginLeft: scale(12) },
                ]}>
                  {item.name}
                </Text>
      ...
      <View style={[styles.ctaBar, { backgroundColor: colors.background }]}>
        {hasClaim ? (
          ...
          <View style={[styles.ctaBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }]}>
            <Text style={[styles.ctaBtnText, T.semibold, { color: colors.tertiaryText }]}>
...
Test on device:
1. Scan Receipt → take photo → parsing animation → **People screen** appears with your avatar + 3 mock members, all selected by default
2. Tap a member to deselect (avatar fades out, "X people splitting $Y each" updates)
3. Tap "Choose My Items" → **Items screen** with stagger entrance
4. Tap items to claim — checkbox fills with accent + spring pop, item name turns accent
5. Tap "Add Tip" → inline text input appears, type a tip amount → summary updates
6. CTA shows "My Share · $X.XX" when anything is claimed, greyed out placeholder when nothing claimed
7. Tap CTA → navigates to Groups tab (no more navigator error)
