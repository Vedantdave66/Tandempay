# Receipt Scan — Apple HIG Polish + UX Clarity
...
Full rewrite of `mobile/src/screens/ReceiptScanScreen.tsx`. Two goals:
1. **Replace the confusing Mine/Shared/None toggle** with a simple circular checkbox per item. Unclaimed items get a "Split remaining equally" toggle at the bottom.
2. **Apple HIG feel**: spring animations, stagger entrance, breathing room, better typography hierarchy, less AI-built.
...
## Design decisions before reading the code

### Claim UX (replaces Mine/Shared/None)
- Each item row has a **circular checkbox** on the right — empty = not yours, filled accent circle with white checkmark = yours
- Tapping an item row anywhere claims/unclaims it with a spring scale pop
- At the bottom of the results, a toggle row: **"Split unclaimed items"** — when ON, anything nobody has claimed gets divided equally (default ON). This replaces the "Shared" concept cleanly
- Your estimated share reflects: sum of your items + (unclaimed total ÷ group size, when split toggle is ON)

### Apple HIG rules applied
- Press feedback: `TouchableOpacity activeOpacity={0.78}` + a `scale: 0.98` spring on the card
- Item entrance: stagger animation — items fly in from `translateY: 16` to `0`, opacity `0 → 1`, `Animated.stagger(45ms)`, cap at first 6 items
- Checkbox: `Animated.spring` scale from 0.6 → 1 on selection (damping 18, stiffness 260)
- Section dividers: `StyleSheet.hairlineWidth` only, never 1px
- Corner radius: item cards `ms(16)`, totals card `ms(20)`, CTA `ms(16)`
- Touch targets: every tappable row `minHeight: scale(52)`
- Shadow: `shadowOpacity` max 0.10 light / 0.22 dark, `shadowRadius` 8, no elevation > 4
- Typography: item name `ms(15)` semibold, price `ms(14)` regular, section labels `ms(12)` uppercase spaced
- "Your share" hero number: `ms(32)` extrabold, centered, accent color
...
export default function ReceiptScanScreen({ navigation }: any) {
  ...
  // Tax + tip prorated to my share of the food total
...
Test on device:
- Idle screen: glow gradient behind camera icon, no "Coming Soon" badge
- Open camera → take photo → scanning animation is smooth, scan line sweeps top to bottom on loop
- Results: items stagger-fly in from below (first 6 items), subsequent items appear instantly
- Tap an item → circular checkbox fills with accent + checkmark + spring pop + card border lights up in accent
- Tap again → unclaims with spring shrink
- "Split unclaimed items" toggle: when ON, remaining items divide among 3 people and are included in your share
- "YOUR ESTIMATED SHARE" hero number updates immediately as you tap
- Share hero is only visible once at least one item is claimed or split toggle is ON
- "Add to Group" CTA enabled as soon as split toggle is ON or any item is claimed
