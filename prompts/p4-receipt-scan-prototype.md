# Receipt Scan — Working Camera Prototype

Load `graphify-out/GRAPH_REPORT.md`. Relevant communities: 25 (Mobile Navigation & Notifications), 16 (UI Primitive Components), 59 (Mobile Pro Hub).

This replaces the "Coming Soon" `ReceiptScanScreen` with a real working prototype: camera opens, user takes a photo, a mock parse result is displayed with per-item claim/share UI. Real OCR backend is wired up in a follow-up prompt. This prompt is mobile-only.
...
`mobile/src/screens/ReceiptScanScreen.tsx` — full replacement below.

---

## ReceiptScanScreen.tsx — full replacement

The screen has three views driven by a `phase` state variable:
- `'idle'` — landing view: description + "Open Camera" button
- `'parsing'` — animated scanning state shown while "OCR" runs (we use a 2.5s setTimeout mock)
- `'results'` — itemized receipt results with per-item claim UI + summary + CTA
...
export default function ReceiptScanScreen({ navigation }: any) {
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        {/* Claim legend */}
        <View style={[styles.legendRow, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', borderRadius: ms(12) }]}>
        ...
        {MOCK_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <View style={[styles.claimBadge, { backgroundColor: isClaimed ? bg : (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)') }]}>
                {claimIcon(mode)}
        ...
        <View style={[styles.totalsCard, {
          ...
          <View style={[styles.totalDivider, { backgroundColor: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)' }]} />
          <View style={styles.totalRow}>
      ...
      <View style={[styles.ctaBar, { backgroundColor: colors.background }]}>
        <TouchableOpacity
          style={[styles.ctaBtn, { backgroundColor: colors.accent, opacity: Object.keys(claims).length === 0 ? 0.4 : 1 }]}
          activeOpacity={0.85}
...
Test on a **physical device** (not simulator — simulator camera returns blank):
1. Tap + FAB → Scan Receipt → "Open Camera" button appears
2. Tap "Open Camera" → iOS camera permission prompt appears → grant it → camera opens
3. Take a photo → screen transitions to "Reading receipt…" with scanning animation for 2.5s
4. Results screen shows 5 parsed items → tap each item to cycle: Unclaimed → Mine (green) → Shared (indigo) → Unclaimed
5. "Your estimated share" chip updates as you claim items
6. Tax + tip totals are shown at the bottom
7. "Add to Group" CTA activates once at least one item is claimed → navigates to Groups

Note: real OCR (sending the photo to the backend) is the next prompt — this validates the entire UX flow works first.
